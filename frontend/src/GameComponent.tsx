...
const GameComponent = ({ onGameWon }) => {
  ...
  useEffect(() => {
    ...
    const ditherPatterns = [
      createDitherPattern(0.25),
      createDitherPattern(0.5),
      createDitherPattern(0.75),
      createDitherPattern(1.0),
    ];
    ...
    function spawnNode() {
      ...
      switch (nodes.length) {
        case 0:
          x = Math.random() * (quadrantWidth - padding * 2) + padding;
          y = Math.random() * (quadrantHeight - padding * 2) + padding;
          break;
        case 1:
          x = quadrantWidth + Math.random() * (quadrantWidth - padding * 2);
          y = Math.random() * (quadrantHeight - padding * 2) + padding;
          break;
        case 2:
          x = quadrantWidth + Math.random() * (quadrantWidth - padding * 2);
          y = quadrantHeight + Math.random() * (quadrantHeight - padding * 2);
          break;
        case 3:
          x = Math.random() * (quadrantWidth - padding * 2) + padding;
          y = quadrantHeight + Math.random() * (quadrantHeight - padding * 2);
    ...
    function getFittingFontSize(text, nodesArr) {
      let sumX = 0,
        sumY = 0;
      let minX = Infinity,
        maxX = -Infinity;
      ...
      const centerX = sumX / nodesArr.length;
      const centerY = sumY / nodesArr.length;
    ...
    // Click-click behavior implementation (click start, click target to complete).
    const clickHandler = (e) => {
      ...
      // If we're already drawing, clicking another node finalizes the link
      if (isDrawingLink && clickedNode && startNode && clickedNode.id !== startNode.id) {
        const endNode = clickedNode;
        const alreadyExists = links.some(
          (link) =>
            (link.nodeA.id === startNode.id && link.nodeB.id === endNode.id) ||
            (link.nodeA.id === endNode.id && link.nodeB.id === startNode.id)
        );
        ...
        // end drawing state after click-to-complete
        isDrawingLink = false;
        startNode = null;
      }
    };
    ...
    if (resizeCanvas()) {
      ...
      spawnNode();
      gameLoop();
      // Attach canvas click handler for click-click behavior and pointermove for transient line rendering
